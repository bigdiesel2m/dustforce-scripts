/* 
Joke script I made as a reference to some DLC 3 memes.
Most the code is similar to my neon.as script, from the same event.
*/
...
class Worth : Protest, trigger_base {
	...
	void draw_it() {
		if (bg_visible) {
			...
			spr2.draw_world(layer, 21, "buildingblocks_8", 0, 0, self.x()-180, self.y()-255 + yoffset, 0.0, 1.1, 0.7, 0xFFFFFFFF);
			tf.draw_world(layer, 22, self.x()-21, self.y()-170 + yoffset, 1.0, 1.0, 0.0);
			tf2.draw_world(layer, 22, self.x()-21, self.y()-140 + yoffset, 1.0, 1.0, 0.0);
		}
	}
}

class Man : Protest, trigger_base {
	uint timer = 0;
	float yoffset = 0;
	float yvelo = -4.5;
	...
	void draw_it() {
		if (bg_visible) {
			spr.draw_world(layer, 20, "victory", 7, 0, self.x(), self.y() + yoffset, 0.0, 1.0, 1.0, 0xFFFFFFFF);
			spr2.draw_world(layer, 20, "buildingblocks_10", 0, 0, self.x()-25, self.y()-90 + yoffset, 185, 0.35, 0.35, 0xFFFFFFFF);
			spr2.draw_world(layer, 21, "buildingblocks_8", 0, 0, self.x()-172, self.y()-225 + yoffset, 5, 1.1, 0.4, 0xFFFFFFFF);
			tf.draw_world(layer, 22, self.x()-20, self.y()-155 + yoffset, 0.95, 0.95, 5.0);
		}
	}
}